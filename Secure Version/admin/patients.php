...
// Fetch all patients with extra info
$stmt = $pdo->query('SELECT u.id, u.username, u.email, p.date_of_birth, p.gender, p.blood_type, u.is_active, u.created_at FROM users u JOIN patients p ON u.id = p.user_id WHERE u.role = "patient" ORDER BY u.id ASC');
...
<body>
    ...
    <section class="py-5">
        <div class="container mt-4">
            ...
            <table class="table table-bordered mt-3 table-custom" id="patientsTable">
                <thead>
                    <tr>
                        <th>ID</th>
                        <th>Username</th>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            <a href="upload_patient_file.php?patient_id=<?= htmlspecialchars($p['id'] ?? '') ?>" class="btn btn-sm btn-primary">Upload File</a>
                        </td>
                    </tr>
                    <?php endforeach; ?>
                </tbody>
    ...
    <script>
    // Client-side search for patients table
