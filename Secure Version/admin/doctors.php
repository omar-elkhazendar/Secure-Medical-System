...
// Fetch all doctors with extra info
$stmt = $pdo->query('SELECT u.id, u.username, u.email, d.specialization, d.license_number, u.is_active, u.created_at FROM users u JOIN doctors d ON u.id = d.user_id WHERE u.role = "doctor" ORDER BY u.id ASC');
...
<body>
    ...
    <section class="py-5">
        <div class="container mt-4">
            ...
            <table class="table table-bordered mt-3 table-custom" id="doctorsTable">
                <thead>
                    <tr>
                        <th>ID</th>
                        <th>Username</th>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td><?= $d['is_active'] ? '<span class="badge bg-success">Active</span>' : '<span class="badge bg-danger">Inactive</span>' ?></td>
    ...
    <script>
    // Client-side search for doctors table
    ...
    document.getElementById('exportCSV').addEventListener('click', function() {
        exportTableToCSV('doctors.csv');
    });
    // Export to PDF
    document.getElementById('exportPDF').addEventListener('click', function() {
        const { jsPDF } = window.jspdf;
        const doc = new jsPDF();
        doc.text('Doctors List', 14, 14);
