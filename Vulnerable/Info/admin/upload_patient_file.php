...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_FILES['patient_file']) && $_FILES['patient_file']['error'] === UPLOAD_ERR_OK) {
        ...
        if (move_uploaded_file($_FILES['patient_file']['tmp_name'], $file_path)) {
            // File is now stored without encryption for easy viewing
            // If you need encryption, use the DocumentHandler class
...
if (isset($_GET['delete_file_id'])) {
    $delete_id = (int)$_GET['delete_file_id'];
    // Get file info
    $stmt = $pdo->prepare('SELECT file_path FROM patient_files WHERE id = ? AND patient_id = ?');
    ...
    if ($file) {
        ...
        $stmt = $pdo->prepare('DELETE FROM patient_files WHERE id = ? AND patient_id = ?');
        $stmt->execute([$delete_id, $patient_id]);
        // Redirect to avoid resubmission
        header('Location: upload_patient_file.php?patient_id=' . $patient_id);
        exit;
    }
}
?>
                                            ...
                                            <?php foreach ($existing_files as $file): ?>
                                                <tr>
                                                    ...
                                                    <td>
                                                        ...
                                                        <a href="upload_patient_file.php?patient_id=<?= $patient_id ?>&delete_file_id=<?= $file['id'] ?>"
