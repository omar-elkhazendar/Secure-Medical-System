...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    // Get raw input for SQL injection testing (no trimming to preserve payloads)
    ...
    // Email validation disabled for SQL injection testing
    ...
    // Check if username or email exists - VULNERABLE TO SQL INJECTION
    $query = "SELECT COUNT(*) FROM users WHERE username = '$username' OR email = '$email'";
    // Debug: Show SQL query (remove in production)
    if (isset($_GET['debug'])) {
    ...
    try {
        ...
        // Show SQL error for debugging SQL injection
        $errors[] = 'SQL Error: ' . $e->getMessage();
        if (isset($_GET['debug'])) {
            $errors[] = 'Query: ' . htmlspecialchars($query);
        }
    }
    ...
    if (empty($errors)) {
        ...
        try {
            // Insert into users with hashed password, role is always 'patient' - VULNERABLE TO SQL INJECTION
            $query = "INSERT INTO users (username, password, email, role, is_active) VALUES ('$username', '$hashed_password', '$email', 'patient', 1)";
            // Debug: Show SQL query (remove in production)
            if (isset($_GET['debug'])) {
            ...
            // Insert into patients - VULNERABLE TO SQL INJECTION
            $query2 = "INSERT INTO patients (user_id, date_of_birth, blood_type) VALUES ($user_id, '$date_of_birth', '$blood_type')";
            if (isset($_GET['debug'])) {
                error_log("INSERT Patients Query: " . $query2);
            }
            $pdo->exec($query2);
                        ...
                        <form method="POST" action="signup.php">
                            ...
                            <div class="mb-3">
                                ...
                                <div class="input-group">
                                    ...
                                    <input type="text" class="form-control" name="email" placeholder="Enter your email" required value="<?= isset($email) ? htmlspecialchars($email) : '' ?>">
                                </div>
                            </div>
                            <div class="mb-3">
