...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        // Vulnerable query - no prepared statements (SQL Injection exists)
        $query = "SELECT * FROM users WHERE email = '$email'";
        // For debugging SQL injection (remove in production)
        if (isset($_GET['debug']) || isset($_POST['debug'])) {
            $error = "<!-- SQL Query: " . htmlspecialchars($query) . " -->";
            error_log("SQL Query: " . $query);
        }
        $result = $pdo->query($query);
        $user = $result->fetch();
        
        // For SQL injection testing: if password is 'bypass' and user exists, allow login
        // This makes it easier to test SQL injection without knowing actual passwords
        $password_bypass = ($password === 'bypass' && $user);
        
        // Check if user exists and has a valid password hash before verifying
        $password_valid = false;
        if ($user && isset($user['password']) && $user['password'] !== null) {
            $password_valid = password_verify($password, $user['password']);
        }
        
        if ($user && ($password_bypass || $password_valid)) {
            $_SESSION['user'] = $user;
        ...
        // Show SQL query in error for debugging
        if (isset($_GET['debug']) || isset($_POST['debug'])) {
            $error .= "<br><small>SQL Query: " . htmlspecialchars($query) . "</small>";
        }
    }
}
?>
