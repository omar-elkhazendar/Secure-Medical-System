...
// Get doctor id
$result = $pdo->query("SELECT id FROM doctors WHERE user_id = '$user[id]'");
$doctor = $result->fetch();
$doctor_id = $doctor['id'];
...
if (
    ...
    $pdo->exec("INSERT INTO posts (doctor_id, content) VALUES ('$doctor_id', \"$content\")");
            ...
            if ($error === UPLOAD_ERR_OK && $size <= 10*1024*1024) { // 10MB max
                ...
                if (in_array($ext, $allowed_ext)) {
                    ...
                    if (move_uploaded_file($tmp_name, $dest)) {
                        ...
                        $web_path = 'uploads/posts/' . $new_name;
                        $file_type = in_array($ext, ['jpg','jpeg','png','gif','bmp']) ? 'image' : 'file';
                        $pdo->exec("INSERT INTO post_attachments (post_id, file_path, file_type) VALUES ('$post_id', '$web_path', '$file_type')");
                    }
                }
            }
        }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['comment_post_id']) && isset($_POST['comment'])) {
    ...
    if ($comment !== '') {
        $pdo->exec("INSERT INTO comments (post_id, doctor_id, comment) VALUES ('$comment_post_id', '$doctor_id', '$comment')");
        header('Location: social_feed.php');
        exit;
    }
}
...
if (isset($_GET['like']) && isset($_GET['post_id'])) {
    ...
    $result = $pdo->query("SELECT id FROM post_likes WHERE post_id = '$post_id' AND doctor_id = '$doctor_id'");
    $like = $result->fetch();
    if ($like) {
        // Unlike
        $pdo->exec("DELETE FROM post_likes WHERE id = '$like[id]'");
    } else {
        // Like
        $pdo->exec("INSERT INTO post_likes (post_id, doctor_id) VALUES ('$post_id', '$doctor_id')");
    }
    header('Location: social_feed.php');
    exit;
}
...
if (isset($_GET['delete_post'])) {
    ...
    $result = $pdo->query("SELECT * FROM posts WHERE id = '$delete_post_id' AND doctor_id = '$doctor_id'");
    if ($result->fetch()) {
        // Delete post (comments and likes will be deleted by ON DELETE CASCADE)
        $pdo->exec("DELETE FROM posts WHERE id = '$delete_post_id'");
    }
    header('Location: social_feed.php');
    exit;
}
...
if (isset($_GET['delete_comment'])) {
    ...
    $result = $pdo->query("SELECT * FROM comments WHERE id = '$delete_comment_id' AND doctor_id = '$doctor_id'");
    if ($result->fetch()) {
        $pdo->exec("DELETE FROM comments WHERE id = '$delete_comment_id'");
    }
    header('Location: social_feed.php');
    exit;
}
...
if ($post_ids) {
    ...
    $result = $pdo->query("SELECT post_id, COUNT(*) as like_count FROM post_likes WHERE post_id IN ($in) GROUP BY post_id");
    foreach ($result->fetchAll() as $row) {
        $likes[$row['post_id']] = $row['like_count'];
    }
    // Get user likes
    $result = $pdo->query("SELECT post_id FROM post_likes WHERE post_id IN ($in) AND doctor_id = '$doctor_id'");
    foreach ($result->fetchAll() as $row) {
...
if ($post_ids) {
    $in = implode(',', array_map('intval', $post_ids));
    $result = $pdo->query("SELECT c.*, u.username, d.specialization FROM comments c JOIN doctors d ON c.doctor_id = d.id JOIN users u ON d.user_id = u.id WHERE c.post_id IN ($in) ORDER BY c.created_at ASC");
    foreach ($result->fetchAll() as $row) {
...
$following_doctors = [];
if ($doctor_ids) {
    $in = implode(',', array_map('intval', $doctor_ids));
    // عدد المتابعين لكل طبيب
    $result = $pdo->query("SELECT doctor_id, COUNT(*) as cnt FROM doctor_followers WHERE doctor_id IN ($in) GROUP BY doctor_id");
    foreach ($result->fetchAll() as $row) {
        $followers_counts[$row['doctor_id']] = $row['cnt'];
    }
    // من أتابعهم أنا
    $result = $pdo->query("SELECT doctor_id FROM doctor_followers WHERE follower_id = '$doctor_id' AND doctor_id IN ($in)");
    foreach ($result->fetchAll() as $row) {
...
if ($post_ids) {
    $in = implode(',', array_map('intval', $post_ids));
    $result = $pdo->query("SELECT * FROM post_attachments WHERE post_id IN ($in)");
    foreach ($result->fetchAll() as $row) {
    ...
    <div class="feed-container">
        ...
        <?php foreach ($posts as $post): ?>
            <div class="post-card" data-post-id="<?php echo $post['id']; ?>">
                ...
                <div class="post-content-view"><?php echo nl2br($post['content']); ?></div>
