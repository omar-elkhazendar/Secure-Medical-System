...
require_once 'includes/mfa.php'; // Add MFA class
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_SESSION['login_attempts']) && $_SESSION['login_attempts'] >= 5) {
        $time_since_last_attempt = time() - $_SESSION['last_login_attempt_at'];
        if ($time_since_last_attempt < 900) { // 15 minutes
            $error = "Too many failed login attempts. Please try again in 15 minutes.";
        } else {
            unset($_SESSION['login_attempts']);
            unset($_SESSION['last_login_attempt_at']);
        }
    }
            ...
            if ($user && !empty($user['password']) && password_verify($password, $user['password'])) {
                ...
                if (empty($error)) {
                    ...
                    if ($secret) {
                        ...
                        logActivity($pdo, $user['id'], 'mfa_required', 'MFA verification required', Auth::getClientIp());
                ...
                $_SESSION['last_login_attempt_at'] = time();
