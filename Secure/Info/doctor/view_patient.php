...
// Get the logged-in doctor's ID FIRST - FIXED: Early doctor ID retrieval
...
// جلب بيانات المريض - FIXED: Check if doctor has access to this patient
...
// FIXED: Verify the doctor has appointments with this patient before showing data
$stmt = $pdo->prepare('
    ...
    AND EXISTS (
        ...
        WHERE doctor_id = ? AND patient_id = p.id AND status IN ("scheduled", "completed")
    )
');
...
if (!$patient) {
    // Redirect back to my_patients if patient not found OR no access
    $_SESSION['error_message'] = 'Patient not found or you do not have access to this patient.';
    header('Location: my_patients.php');
    exit;
}
...
// جلب السجلات الطبية - FIXED: Only show records for this patient AND created by current doctor
$stmt = $pdo->prepare('
...
');
$stmt->execute([$patient['id'], $doctor_id]);
