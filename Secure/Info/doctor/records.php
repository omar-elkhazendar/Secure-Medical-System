...
$stmt = $pdo->prepare('SELECT mr.*, p.id as patient_id, u.username as patient_name FROM medical_records mr JOIN patients p ON mr.patient_id = p.id JOIN users u ON p.user_id = u.id WHERE mr.doctor_id = ? ORDER BY mr.created_at DESC');
