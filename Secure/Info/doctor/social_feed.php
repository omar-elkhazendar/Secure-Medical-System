...
// Security headers
header("Content-Security-Policy: default-src 'self'; script-src 'self' 'unsafe-inline' https://cdn.jsdelivr.net https://cdnjs.cloudflare.com; style-src 'self' 'unsafe-inline' https://cdn.jsdelivr.net https://cdnjs.cloudflare.com https://fonts.googleapis.com; img-src 'self' data:; font-src 'self' https://fonts.gstatic.com;");
...
function secureFileUpload($file, $upload_dir, $allowed_extensions) {
    $errors = [];
    ...
    if (move_uploaded_file($file['tmp_name'], $dest)) {
        ...
        return [
            'success' => true,
            'file_path' => 'uploads/posts/' . $new_name,
            'file_name' => $new_name,
            'file_type' => in_array($file_ext, ['jpg','jpeg','png','gif','bmp']) ? 'image' : 'file'
        ];
    } else {
...
if (
    ...
    if ($content !== '') {
        ...
        if (!empty($_FILES['attachments']['name'][0])) {
            ...
            $htaccess_content = "RemoveHandler .php .phtml .php3 .php4 .php5 .php6 .phps .cgi .exe .pl .asp .aspx .shtml .shtm .fcgi .fpl .jsp\n";
            $htaccess_content .= "RemoveType .php .phtml .php3 .php4 .php5 .php6 .phps .cgi .exe .pl .asp .aspx .shtml .shtm .fcgi .fpl .jsp\n";
            $htaccess_content .= "<FilesMatch \"\\.(php|phtml|php3|php4|php5|php6|phps|cgi|pl|asp|aspx|shtml|shtm|fcgi|fpl|jsp)$\">\n";
            ...
            foreach ($_FILES['attachments']['name'] as $i => $name) {
                ...
                $upload_result = secureFileUpload($file, $upload_dir, $allowed_ext);
                
                if ($upload_result['success']) {
                    // Store with prepared statement
                    $stmt = $pdo->prepare('INSERT INTO post_attachments (post_id, file_path, file_type) VALUES (?, ?, ?)');
                    $stmt->execute([$post_id, $upload_result['file_path'], $upload_result['file_type']]);
                } else {
                    // Log upload errors
                    foreach ($upload_result['errors'] as $error) {
                        error_log("File upload error: " . $error . " for file: " . $file['name']);
                    }
                }
            }
        }
...
function executeInQuery($pdo, $base_query, $ids, $additional_params = []) {
    if (empty($ids)) {
        return [];
    }
    
    $placeholders = str_repeat('?,', count($ids) - 1) . '?';
    $query = str_replace('IN_CLAUSE', $placeholders, $base_query);
    $params = array_merge($ids, $additional_params);
...
if ($post_ids) {
    // Get like counts
    $like_counts = executeInQuery($pdo, 
        "SELECT post_id, COUNT(*) as like_count FROM post_likes WHERE post_id IN (IN_CLAUSE) GROUP BY post_id",
        $post_ids
    );
    ...
    // Get user likes
    $user_like_data = executeInQuery($pdo,
        "SELECT post_id FROM post_likes WHERE post_id IN (IN_CLAUSE) AND doctor_id = ?",
        $post_ids,
        [$doctor_id]
    );
    foreach ($user_like_data as $row) {
...
$comments = [];
if ($post_ids) {
    $comment_data = executeInQuery($pdo,
        "SELECT c.*, u.username, d.specialization FROM comments c JOIN doctors d ON c.doctor_id = d.id JOIN users u ON d.user_id = u.id WHERE c.post_id IN (IN_CLAUSE) ORDER BY c.created_at ASC",
        $post_ids
    );
...
if ($doctor_ids) {
    ...
    $follower_data = executeInQuery($pdo,
        "SELECT doctor_id, COUNT(*) as cnt FROM doctor_followers WHERE doctor_id IN (IN_CLAUSE) GROUP BY doctor_id",
        $doctor_ids
    );
    ...
    // من أتابعهم أنا
    $following_data = executeInQuery($pdo,
        "SELECT doctor_id FROM doctor_followers WHERE follower_id = ? AND doctor_id IN (IN_CLAUSE)",
        $doctor_ids,
        [$doctor_id]
    );
...
$post_attachments = [];
if ($post_ids) {
    $attachment_data = executeInQuery($pdo,
        "SELECT * FROM post_attachments WHERE post_id IN (IN_CLAUSE)",
        $post_ids
    );
                        ...
                        <?php foreach ($post_attachments[$post['id']] as $att): ?>
                            <?php if ($att['file_type'] === 'image'): ?>
                                <a href="../<?php echo htmlspecialchars($att['file_path']); ?>" target="_blank" class="post-image-container">
                                    <img src="../<?php echo htmlspecialchars($att['file_path']); ?>" alt="Uploaded image" class="post-image">
                                </a>
                            <?php else: ?>
                                <a href="../<?php echo htmlspecialchars($att['file_path']); ?>" target="_blank" class="btn btn-outline-secondary btn-sm">
                                    <i class="fas fa-paperclip"></i> <?php echo htmlspecialchars(basename($att['file_path'])); ?>
                                </a>
                            <?php endif; ?>
                        <?php endforeach; ?>
                    </div>
